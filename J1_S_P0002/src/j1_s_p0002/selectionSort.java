...
public class selectionSort {
    ...
    /**
     * Use the principle of selection sort to sort the array from smallest to
     * largest
     *
    ...
    public void sorting(int[] arr) {
        int n = arr.length;
        // Traverse from beginning to n-2
        for (int i = 0; i < n - 1; i++) {
            // Find the smallest element in the range from i+1 to the end of the array
            int minIndex = i;
            for (int j = i + 1; j < n; j++) {
            ...
            // Swap the smallest element with the [i]
            int temp = arr[i];
            arr[i] = arr[minIndex];
            arr[minIndex] = temp;
        }
